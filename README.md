...
[GoCD agent](https://www.gocd.org) docker image based on ArchLinux.
...
```
docker run -d -e GO_SERVER_URL=... tecladozurdo/gocd-agent-archlinux:latest
```
...
```
docker run -d -e GO_SERVER_URL=http://$(docker inspect --format='{{(index (index .NetworkSettings.IPAddress))}}' angry_feynman):8153/go tecladozurdo/gocd-agent-archlinux:latest
```
OR
...
```
docker run -d -e GO_SERVER_URL=http://<ip_of_host_machine>:$(docker inspect --format='{{(index (index .NetworkSettings.Ports "8153/tcp") 0).HostPort}}' angry_feynman)/go tecladozurdo/gocd-agent-archlinux:latest
```
...
docker run -d \
        ...
        tecladozurdo/gocd-agent-archlinux:latest
```
...
```
docker run -v /path/to/godata:/godata -v /path/to/home-dir:/home/go tecladozurdo/gocd-agent-archlinux:latest
```
...
```
docker run -e GOCD_AGENT_JVM_OPTS="-Dfoo=bar" tecladozurdo/gocd-agent-archlinux:latest
```
